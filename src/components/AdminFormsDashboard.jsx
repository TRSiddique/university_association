...
export default function AdminFormsDashboard() {
  ...
  const fetchFormsAndStats = async () => {
    try {
      ...
      const responseCounts = await Promise.all(
        data.map(form => 
          fetch(`university-association-backend-1.onrender.com/api/admin/forms/${form._id}/responses`)
            .then(res => res.json())
  ...
  const toggleFormStatus = async (id, currentStatus) => {
    try {
      const response = await fetch(`university-association-backend-1.onrender.com/api/admin/forms/${id}`, {
        method: 'PUT',
